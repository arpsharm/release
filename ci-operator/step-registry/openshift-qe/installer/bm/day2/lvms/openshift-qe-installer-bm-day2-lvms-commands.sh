...
wipe_nvme() {
  ...
  ssh ${SSH_ARGS} root@${bastion} '
    for i in $(oc get node --no-headers -l node-role.kubernetes.io/worker --output custom-columns="NAME:.status.addresses[0].address"); do
      ssh -t '"${INNER_SSH_ARGS}"' core@$i "sudo vgchange -an 2>/dev/null || true" 2>/dev/null
      for j in {0..7}; do
        ssh -t '"${INNER_SSH_ARGS}"' core@$i "test -b /dev/nvme${j}n1 && { sudo pvremove -ff -y /dev/nvme${j}n1 2>/dev/null || true; sudo sgdisk --zap-all /dev/nvme${j}n1 || true; sudo wipefs -a /dev/nvme${j}n1 || true; }" 2>/dev/null
      done
    done'
}
  ...
  ssh ${SSH_ARGS} root@${bastion} '
    for i in $(oc get node --no-headers -l node-role.kubernetes.io/worker --output custom-columns="NAME:.status.addresses[0].address"); do
      ...
      for d in sdb sdc sdd sde sdf sdg sdh; do
        ssh -t '"${INNER_SSH_ARGS}"' core@$i "lsblk /dev/$d -no MOUNTPOINT 2>/dev/null | grep -q . || { sudo pvremove -ff -y /dev/$d 2>/dev/null || true; sudo sgdisk --zap-all /dev/$d 2>/dev/null; sudo wipefs -a /dev/$d 2>/dev/null; } || true" 2>/dev/null
      done
    done'
}
